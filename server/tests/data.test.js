...
describe("SC Mat Team Data", () => {
	...
	it("should return return an array of items", async () => {
		...
		// ********** When
		const response = await data.scmatTeamGet();

		// ********** Then
		expect(response.status).toEqual(200);
		expect(response.data).toHaveProperty("scmatTeams");
	});
	...
	it("should create a new object", async () => {
		...
		// ********** When
		const response = await data.scmatTeamSave(newData);
	...
	it("should get the new object by id", async () => {
		...
		// ********** When
		const response = await data.scmatTeamGet({ id: createdId });

		// ********** Then
		expect(response.status).toEqual(200);
		expect(response.data).toHaveProperty("scmatTeams");
		expect(response.data.scmatTeams).toHaveLength(1);

		expect(response.data.scmatTeams).toEqual(
			expect.arrayContaining([
				expect.objectContaining(newData)
			])
		);
	...
	it("should return an empty array for non-existant object", async () => {
		...
		// ********** When
		const response = await data.scmatTeamGet({ id: "abcd" });

		// ********** Then
		expect(response.status).toEqual(200);
		expect(response.data).toHaveProperty("scmatTeams");
		expect(response.data.scmatTeams).toHaveLength(0);
	});
	...
	it("should delete the new object", async () => {
		...
		// ********** When
		const response = await data.scmatTeamDelete(createdId);
	...
	it("should return an empty array after deleting the new object", async () => {
		...
		expect(response.status).toEqual(200);
		expect(response.data).toHaveProperty("scmatTeams");
		expect(response.data.scmatTeams).not.toEqual(
			expect.arrayContaining([
				expect.objectContaining({
					id: createdId
				})
