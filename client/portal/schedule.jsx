		...
		<div className={`schedule container ${ pageActive ? "active" : "" }`}>
			...
			<div key={ event.id } className={`panel ${ event.type }`}>
				...
				<div className="row">
					...
					<button aria-label="External Event" className="action" onClick={ () => window.open(event.type === "track" ? `https://www.trackwrestling.com/tw/opentournaments/VerifyPassword.jsp?tournamentId=${ event.trackId}` : `https://events.flowrestling.org/event/${ event.floGUID }/summary`) }>
						{/* World */}
