...
import "./include/teamduplicate.css";

const TeamDuplicateManagement = () => {
	...
	// Currently selected wrestler candidate lookup record
	...
	const handleExecuteSearch = async (submitEvent) => {
		...
		try {
			...
			setErrorMessage(error.message || "Failed to perform team search.");
			setSearchWrestlerResults([]);
		}
		finally {
	...
	const handleLookupDuplicatesForWrestler = async (wrestlerRecord) => {
		...
		try {
			...
			if (responseData.error) {
				...
				const candidateDuplicates = fetchedWrestler?.potentialDuplicates || [];
		...
		<div className="page">
			...
			<div style={{ minWidth: 0 }}>
				{ isLoading ? (
					...
					<div className="teamduplicate-container">
						<header>
							<h1>Duplicate Team Search</h1>
						</header>
						...
						{/* View 1: Team Wrestlers Search Results */}
						{ activeViewMode === "search_results" && searchWrestlerResults && (
							<div className="search-results-section">
								<h2 className="section-sub-heading">
									Wrestlers in { selectedTeamName } ({ searchWrestlerResults.length })
								</h2>

								{ searchWrestlerResults.length === 0 ? (
									<div className="no-records-message">No wrestlers found for the selected team.</div>
								) : (
						...
						{/* View 2: Candidate Duplicates View */}
									...
									return (
										<div className={`wrestler-duplicate-group-card ${ isGroupSubmitted ? "submitted-card" : "" }`}>
											...
											<table className="candidate-matrix-table desktop-only">
												...
												<tbody>
													{ allGroupCandidates.map((wrestlerCandidate, candidateIndex) => {
														const isCurrentPrimary = Boolean(selectedPrimary && selectedPrimary.sqlId === wrestlerCandidate.sqlId);
														const isCurrentDuplicate = Boolean(selectedDuplicates && selectedDuplicates.some(duplicateCandidate => duplicateCandidate.sqlId === wrestlerCandidate.sqlId));
...
if (rootElement) {
	...
	reactRoot.render(<TeamDuplicateManagement />);
}

export default TeamDuplicateManagement;
